...
const handleStep1=async ()=>{
    ...
    setErr("")
    try {
        const result=await axios.post(`${import.meta.env.VITE_SERVER_URL}/api/auth/sendOtp`,{email},{withCredentials:true})
...
const handleStep3=async ()=>{
    ...
    try {
        ...
        const result=await axios.post(`${import.meta.env.VITE_SERVER_URL}/api/auth/resetPassword`,{email,password:newPassword},{withCredentials:true})
        console.log(result.data)
        setLoading(false)
    } catch (error) {
        console.log(error)
